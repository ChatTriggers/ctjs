...
class Console(val loader: ILoader?) {
    private val frame = JFrame(
        "ChatTriggers ${Reference.MOD_VERSION} ${loader?.getLanguage()?.langName ?: "Default"} Console"
    )
    ...
    companion object {
        val FIRA_FONT: Font = Font.createFont(
            ...
            System.getProperty("ct.firaFile", null)?.let { File(it).inputStream() }
                ?: this::class.java.getResourceAsStream("/assets/chattriggers/FiraCode-Regular.otf")
        ).deriveFont(9f)
